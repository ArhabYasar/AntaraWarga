    ...
    <div class="main-konten">
        <div class="around">
          ...
          <h1 class="kanan"><img src="{{ asset('img/383.png') }}" alt="" class="icon" i class="fas fa-angle-right dropdown"></i>
                ...
                <p class="Data">TERIMA IURAN<a href="{{ url('/dashboardPengurus/TerimaIuran/create') }}" class="btn btn-primary">Tambah Data</a></p>
                <p class="nice">-</p>

                <div class="pencarian">
                    <form action="{{ url('/dashboardPengurus/TerimaIuran') }}" method="get">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $item->nominal }}</td>
                        <td>
                        <button id="openModalBtn" class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#myModal{{ $item->id }}">Detail</button>
                          <div id="myModal{{$item->id}}" class="modal">
                            <div class="modal-content">
                              <b class="sow"> Detail Terima Iuran </b>
                              <span class="close" id="closeModalBtn">&times;</span>
                                  <p> Nama Iuran <span class="kick">:</span>{{ $item->iuran->nama }}<br>
                                    Nama Warga <span class="slow">:</span>{{ $item->warga->nama }}<br>
                                    Tanggal   <span class="denim">:</span>{{ $item->tanggal }}<br>
                                    Nominal <span class="sid">:</span>{{ $item->nominal }}<br>
                                    Penerima Kas <span class="sud">:</span>{{ $item->penerima }}</p>
                            </div>
                          </div>
                            <a href='{{ url('/dashboardPengurus/TerimaIuran/'.$item->id.'/edit') }}' class="btn btn-warning btn-sm">Edit</a>
                            <form action="{{ url('/dashboardPengurus/TerimaIuran/'.$item->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
