...
.modal-content {
  columns: auto; /* Menggunakan auto untuk membiarkan browser memutuskan jumlah kolom */
  column-gap: 20px; /* Jarak antar kolom jika diperlukan */
}

.modal-content p {
  break-inside: avoid; /* Menghindari paragraf terpotong di antara kolom */
}
...
.sud{
  margin-left: 70px;
  margin-right: 10px;
  column-count: 2; /* Atur jumlah kolom sesuai kebutuhan */
    column-gap: 20px; /* Jarak antar kolom jika diperlukan */
...
<div class="container">
  ...
  <div class="sidebar">
    ...
    <div class="main-konten">
        <div class="around">
          <h1 class="strip">senin 19 desember | 11:41</h1>
          <h1 class="kanan"><img src="{{ asset('img/383.png') }}" alt="" class="icon" i class="fas fa-angle-right dropdown"></i>
                ...
                <p class="Data">TAMU MENGINAP<a href="{{ url('/dashboardPengurus/TamuMenginap/create') }}" class="btn btn-primary">Tambah Data</a></p>
                <p class="nice">-</p>

                <div class="pencarian">
                    <form action="{{ url('/dashboardPengurus/TamuMenginap') }}" method="get">
...
<div id="openModalBtn{{ $item->id }}" class="modal">
  <div class="modal-content">
    <b class="sow"> Detail Iuran</b>
    <span class="close" id="closeModalBtn">&times;</span>
    <p> Tamu Menginap <span class="kick">:</span> {{ $item->nama }}<br>
        Tanggal Menginap <span class="slow">:</span>{{ $item->nama }}<br>
        Tempat Menginap    <span class="denim">:</span>{{ $item->getHunian->alamat }}<br>
        Status    <span class="sid">:</span>{{ $item->status }}<br>
        Keperluan <span class="sud">:</span>{{ $item->keperluan }}</p>
  </div>
</div>
                            <a href='{{ url('/dashboardPengurus/TamuMenginap/'.$item->id.'/edit') }}' class="btn btn-warning btn-sm">Edit</a>
                            <form action="{{ url('/dashboardPengurus/TamuMenginap/'.$item->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
