...
<div class="container">
  ...
  <div class="main-konten">
    ...
    <span class="Data">BERITA KAMPUNG > ADD</span>
    <p class="nice">-</p>
    </div>

    <div class="tableone">
        <form action="{{ url('/dashboardPengurus/Berita/'.$data->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="hunian">
            <label for="judul" class="team">Katagori Berita</label>
            <div class="col-sm-10">
                <input type="text" class="form-control" name='kategori' value="{{ $data->kategori }}" id="nik">
            </div>
        </div>
        <div class="hunian">
            <label for="nama" class="team">Judul Berita</label>
            <div class="col-sm-10">
                <input type="text" class="form-control" name='judul' value="{{$data->judul}}" id="nama"></input>
            </div>
        </div>
        <div class="hunian">
            <label for="nama" class="team">Isi Berita</label>
            <div class="col-sm-10">
            <textarea type="text" class="form-control" name='isi' id="nama">{{ $data->isi }}</textarea>
            </div>
        </div>
        @if ($data->foto)
        <div class="hunian">
            <label for="formFile" class="team">Foto Produk</p>
            <div class="hadeh">
                <input type="file" class="kids" id="customFile" name='foto'>
            </div>
            <a class="file"><img src="{{ asset('path-to-your-image-directory/' . $data->foto) }}" alt="" class="icon" width="200px" height="200px" ></i>
            </div>
            @endif
